...
export default function SalesBySKU(tableList, loading) {
  return <div className="row fadeInRight">
            ...
            <table className="table table-row-dashed fs-7 table-row-gray-300 align-middle gx-4 gy-4">
              <thead>
                ...
                >
                  ...
                  <th className="min-w-75px">{loading ? title2Loading() : 'CHG'}</th>
                  <th className="min-w-75px">{loading ? title2Loading() : 'CHG %'}</th>
                  <th
                    className="min-w-75px"
                    style={{ borderLeft: "1px dashed #ddd" }}
                ...
                {loading ?
                  ...
                  : tableList?.map((d, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        {d?.week_start_date || ''} to {d?.week_end_date || ''}</td>
                      <td>${d?.twSales || '0'}</td>
                      <td>${d?.salesDiff || '0'}</td>
                      <td className={`${d?.salesChange < 0 ? 'text-danger' : 'text-success'}`}>{d?.salesChange || '0'}%</td>
                      <td>{d?.twUnits || '0'}</td>
                      <td>{d?.unitsDiff || '0'}</td>
                      <td className={`${d?.unitsChange < 0 ? 'text-danger' : 'text-success'}`}>{d?.unitsChange || '0'}%</td>
                      <td>${d?.lastYearSales || '0'}</td>
                      <td>${d?.salesDiffLy || '0'}</td>
                      <td className={`${d?.salesChangeLy < 0 ? 'text-danger' : 'text-success'}`}>{d?.salesChangeLy || '0'}%</td>
                      <td>{d?.lastYearUnits || '0'}</td>
                      <td>{d?.unitsDiffLy || '0'}</td>
                      <td className={`${d?.unitsChangeLy < 0 ? 'text-danger' : 'text-success'}`}>{d?.unitsChangeLy || '0'}%</td>
                    </tr>
                  ))}
              </tbody>
            </table>
