...
} from '@/src/services/planning.services';
                    ...
                    <table className="table table-row-dashed fs-7 table-row-gray-300 align-middle gx-4 gy-4">
                      <thead>
                        <tr className="text-start text-gray-800 fw-boldest fs-7 ">
                          ...
                          <th className="min-w-150px">Date (for Testing)</th>
                          {
                            selectedColumnsList?.map((d, i) => (
                              <th className="min-w-150px">{d}</th>
                            ))
                          }
                        </tr>
                      </thead>
                      <tbody className="fw-bold text-gray-800 ">
                        {
                          dataList?.map((d, i) => (
                            <tr>
                              ...
                              <td>{d?.log_date}</td>
                              {
                                selectedColumnsList?.map((r, t) => {
                                  const index = ['vendor', 'moq', 'inventory_multiplier', 'unit_per_case'].findIndex((f) => f === r)
                                  if (index !== -1) {
                                    return <td>
                                      <Input placeholder={r} value={d?.[r] || 'not set'} />
                                    </td>
                                  }
                                  return (
                                    <td key={t}>{d?.[r] || '-'}</td>
