import { Button, Divider, Dropdown, message, Select, Space, theme } from 'antd';
...
import VendoTooltip from '../../tooltip';
...
export default function Product(props) {
  ...
  const SalesByProductListRes = useSelector(
    (state) => state.product?.salesByProductList || {}
  );
  const SaveColumnConfigurationRes = useSelector(
    (state) => state.product?.saveColumnConfiguration || {}
  );
  const SalesByProductColumnListRes = useSelector(
    (state) => state.product?.salesByProductColumnList || {}
  );
  const SaveTableConfigurationRes = useSelector(
    (state) => state.product?.saveTableConfiguration || {}
  );
                    ...
                    >
                      ...
                      <tbody className='text-gray-700 fw-bold'>
                        {Object.values(list || {})?.map((d, i) => {
                          const defaultWeek = Object.entries(d || {})?.[0]?.[1];
                          return (
                            <tr key={i}>
                              <td colspan='2'>
                                <div className='fs-7'>
